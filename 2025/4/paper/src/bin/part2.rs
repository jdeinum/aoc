...
fn part2(grid_str: &str) -> Result<usize> {
    // convert grid into paper grid
    let grid = parse_papers(&grid_str).context("parse grid")?;

    // create a proper grid
    let mut padded_grid = PaddedPaperGrid::build(grid).context("build padded grid")?;

    // create the resulting set
    let mut res: HashSet<(usize, usize)> = HashSet::new();

    loop {
        let positions = padded_grid.get_accessible().context("get accessible")?;
        ...
        if positions.is_empty() {
            ...
            positions
                .iter()
                .for_each(|x| padded_grid.set(x.0, x.1, false));
...
fn main() -> Result<()> {
    ...
    let res = part2(&s).context("part 2")?;
...
.@@@@@@@@.
@.@.@@@.@.";
        assert_eq!(part2(input).context("part 2")?, 43);
        Ok(())
    }
}
