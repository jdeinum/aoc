...
fn part1(input: &str, num_connections: usize) -> usize {
    ...
    // find the 3 biggest trees and multiply them
    // Only root nodes have accurate component sizes
    (0..nodes.len())
        .filter(|&i| uf.parent[i] == i) // Only include roots
        .map(|i| uf.size[i])
